...
export function ExistingEntryPicker({
  ...
  const t = useTranslations('Writer.ExistingEntryPicker')
  ...
  useEffect(() => {
    ...
    fetch(`/api/writer/library?documentType=${encodeURIComponent(selectedType)}&term=${encodeURIComponent(query)}`)
            ...
            >
              <div className='flex items-start justify-between gap-4'>
                <div>
                  ...
                  <div className='mt-1 text-xs uppercase tracking-[0.2em] text-muted-foreground'>{item.slug || item.id}</div>
                </div>
