...
describe('MagicsPage', () => {
  ...
  it('switches sections with buttons without refetching', async () => {
    ...
    expect(screen.getByRole('button', { name: '原理' })).toHaveAttribute(
      'aria-pressed',
      'false'
    )
    expect(screen.getByRole('button', { name: '法术' })).toHaveAttribute(
      'aria-pressed',
      'false'
    )
    expect(screen.queryByText('Filters')).not.toBeInTheDocument()
    expect(screen.queryByText('Showing 3 of 3')).not.toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '原理' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '法术' })).toBeInTheDocument()
    expect(fetchMock).toHaveBeenCalledTimes(1)

    await user.click(screen.getByRole('button', { name: '原理' }))

    expect(screen.getByRole('heading', { name: '原理' })).toBeInTheDocument()
    expect(screen.queryByRole('heading', { name: '法术' })).not.toBeInTheDocument()
    expect(screen.queryByText('Filters')).not.toBeInTheDocument()
    expect(
    ...
    await user.click(screen.getByRole('button', { name: '法术' }))

    expect(screen.getByText('元素筛选')).toBeInTheDocument()
    ...
    const fireFilter = document.querySelector<HTMLButtonElement>(
      'button[data-element-filter="fire"]'
    )
    if (!fireFilter) {
    ...
    expect(
      screen.queryByRole('link', { name: /Tidal Ring/i })
    ).not.toBeInTheDocument()
  ...
  it('shows only principle section when filtered by search params', async () => {
    ...
    expect(screen.getByRole('heading', { name: '原理' })).toBeInTheDocument()
    expect(screen.queryByRole('heading', { name: '法术' })).not.toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Ley Theory/i })).toHaveAttribute(
      'href',
      '/magics/ley-theory'
    )
  })
  ...
  it('supports keyboard interaction for spell element filters', async () => {
    ...
    const spellTab = screen.getByRole('button', { name: '法术' })
    ...
    const fireElementButton = document.querySelector<HTMLButtonElement>(
      'button[data-element-filter="fire"]'
    )
  ...
  it('shows spell section and element filters when kind=spell is provided', async () => {
    ...
    expect(
      screen.queryByRole('link', { name: /Ley Theory/i })
    ).not.toBeInTheDocument()
  ...
  it('falls back to all sections when kind is invalid', async () => {
    ...
    expect(screen.getByRole('link', { name: /Arcane Bolt/i })).toHaveAttribute(
      'href',
      '/magics/arcane-bolt'
    )
    expect(screen.getByRole('link', { name: /Ley Theory/i })).toHaveAttribute(
