...
describe("SearchPage", () => {
  ...
  it("renders search results and calls sanity fetch with wildcard term", async () => {
    ...
    expect(screen.getByText("“arthur king” 共找到 1 条结果")).toBeInTheDocument()
  ...
  it("shows empty state when no result matched", async () => {
    ...
    expect(
      screen.getByText("没有匹配结果，请尝试更短或更通用的关键词。")
    ).toBeInTheDocument()
  })
  ...
  it("renders kind/element labels for magic and organization results", async () => {
    fetchMock.mockResolvedValue([
      {
        _id: "magic-1",
        _type: "magic",
    ...
    expect(screen.getByText("法术 · 火")).toBeInTheDocument()
