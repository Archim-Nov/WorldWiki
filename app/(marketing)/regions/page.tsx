    ...
    <div className={styles.page}>
      <div className="container mx-auto px-4 py-12 sm:py-16">
        <ScrollReveal as="header" className="max-w-2xl mb-12">
          ...
          <p className={styles.lead}>以地图与场景为线索，进入每一块世界碎片。</p>
        </ScrollReveal>

        {regions.length === 0 ? (
          <p className={styles.empty}>暂无区域内容，请先在 Studio 中创建。</p>
        ) : (
            ...
            {regions.map((region) => (
              ...
              >
                ...
                <div className={styles.cardContent}>
                  ...
                  {region.country?.name && <p className={styles.cardMeta}>{region.country.name}</p>}
                  {region.summary && <p className={styles.cardSummary}>{region.summary}</p>}
                </div>
              </Link>
            ))}
          </div>
